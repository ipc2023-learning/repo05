...
goal_reward         : LEFTPAR GOAL_REWARD NUMBER RIGHTPAR
                      { problem->set_goal_reward(*($3)); } 
                    ;
...
orbit_decl          : LEFTPAR ORBIT NAME atoms RIGHTPAR
                    {
                      std::cout << "System declation was removed from the parser. Quitting\n";
                      assert(false);
...
make_problem( const std::string *name, const std::string *domain_name )
{
  ...
  if (requirements->rewards) {
    const Application& reward_appl = Application::make_application(
                                              reward_function, TermList());
    const Assignment* reward_assignment =	new Assignment(
                Assignment::ASSIGN_OP, reward_appl, *new Value(0), true);
			...
			const Expression& expr )
{
  ...
  const Assignment& assignment = *new Assignment(oper, application, expr, reward_assignment);
