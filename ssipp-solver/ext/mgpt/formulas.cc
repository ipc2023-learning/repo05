...
Equality::flatten( bool negated ) const
{
  ...
  DIE(false, "Unsupported formula (no implementation)", 138);
  return flatten(negated); // Just to shut-up gcc
}
    ...
    for( hi = hash.begin(); hi != hash.end(); ++hi )
    {
      ...
      if( fvars1.size() == fvars2.size() )
      {
        ...
        if( match( *(*hi).first, subst ) )
        {
          ...
          TermList tmp;
          for (VariableList::const_iterator iv = fvars1.begin();
               iv != fvars1.end(); ++iv)
          {
            tmp.push_back(*iv);
          }
          const Atom *atom = &Atom::make_atom(p, tmp);
