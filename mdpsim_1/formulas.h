...
enum class Truthiness : int {
  TAUTOLOGY, CONTRADICTION, INDETERMINATE
};


// Boolean operators work as you'd expect for T/F/unknown logic
Truthiness operator&&(Truthiness a, Truthiness b);
Truthiness operator||(Truthiness a, Truthiness b);
Truthiness operator!(Truthiness a);
// enforces false < unknown < true ordering
bool operator<(Truthiness a, Truthiness b);
std::ostream& operator<<(std::ostream &s, Truthiness t);
...
struct StateFormula : public RCObject {
  ...
  bool tautology() const { return truthiness() == Truthiness::TAUTOLOGY; }

  /* Tests if this state formula is a contradiction. */
  bool contradiction() const { return truthiness() == Truthiness::CONTRADICTION; }

  /* Whether state formula is statically true, statically false, or something else */
  virtual Truthiness truthiness() const { return Truthiness::INDETERMINATE; };
  ...
  /* Tests if this state formula holds in the given state. */
  virtual bool holds_noValues(const TermTable& terms,
              const AtomSet& atoms) const = 0;


  /* Enumerate all atoms appearing in this state formula */
  virtual void listAtoms(AtomSet& atomsS,
                         AtomList& atomsL,
                         bool dupes=false) const = 0;

  /* Progress estimator. */
  virtual int progress(const TermTable& terms,
              const AtomSet& atoms, const ValueMap& values) const = 0;
  ...
  virtual const StateFormula& instantiation(const SubstitutionMap& subst,
    ...
    //virtual void randomState(State* s, const Problem& problem) const = 0;
...
/*
 * Wraps a StateFormula so that it always gives back a certain truthiness value.
 * Good for short-circuiting evaluation, and for detecting whether certain
 * conditions are *ever* true (within a given problem)
 */
struct TruthyWrapper : public StateFormula {
  static const StateFormula &make(const StateFormula &wrapped_, Truthiness truthiness_);

  /* Delete delicious truthy thing inside the wrapper */
  virtual ~TruthyWrapper();

  /* Tests if this state formula holds in the given state. */
  virtual bool holds(const TermTable& terms, const AtomSet& atoms, const
  ...
  /* Whether state formula is statically true, statically false, or something else */
  virtual Truthiness truthiness() const { return truthiness_; };

private:
  Truthiness truthiness_;
  const StateFormula *wrapped_;
  explicit TruthyWrapper(const StateFormula &wrapped_, Truthiness truthiness_);
};
...
struct Comparison : public StateFormula {
  ...
  virtual void listAtoms(AtomSet& atomsS, AtomList &atomsL, bool dupes=false)
    const;
 protected:
...
struct Negation : public StateFormula {
  ...
  /* Whether state formula is statically true, statically false, or something else */
  virtual Truthiness truthiness() const {return !(negand_->truthiness());};
...
struct Conjunction : public StateFormula {
  ...
  /* Whether state formula is statically true, statically false, or something else */
  virtual Truthiness truthiness() const;
