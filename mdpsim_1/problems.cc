...
/* Fills the given list with actions enabled in the given state. */
/* Actions taken in the list "remainingActions", and values not taken into account. */
void Problem::enabled_actions_noValues(
				       ActionSet& remainingActions,
				       ActionList& actions,
				       const AtomSet& atoms
				       ) const {
  for (ActionSet::const_iterator ai = remainingActions.begin();
       ai != remainingActions.end(); ai++) {
      if ((*ai)->enabled_noValues(terms_, atoms)) {
      actions.push_back(*ai);
    }
  }
}
