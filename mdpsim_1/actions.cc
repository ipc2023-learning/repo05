...
/* Tests if this action is ever enabled in the given state. */
/* (don't care about values) */
bool Action::enabled_noValues(const TermTable& terms,
		     const AtomSet& atoms) const {
    return precondition_->holds_noValues(terms, atoms);
}
