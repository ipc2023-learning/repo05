...
void ScaleUp::affect(ValueMap& values) const {
  ...
  if (vi == values.end()) {
    ...
    msg << "changing undefined value in ScaleUp::affect: " << fluent();
...
void ScaleDown::affect(ValueMap& values) const {
  ...
  if (vi == values.end()) {
    std::ostringstream msg;
    msg << "changing undefined value in ScaleDown::affect: " << fluent();
    throw std::logic_error(msg.str());
  } else {
    values[&fluent()] = (*vi).second / expression().value(values);
  }
...
void Increase::affect(ValueMap& values) const {
  ...
  if (vi == values.end()) {
    std::ostringstream msg;
    msg << "changing undefined value in Increase::affect: " << fluent();
    throw std::logic_error(msg.str());
  } else {
    values[&fluent()] = (*vi).second + expression().value(values);
  }
...
void Decrease::affect(ValueMap& values) const {
  ...
  if (vi == values.end()) {
    std::ostringstream msg;
    msg << "changing undefined value in Decrease::affect: " << fluent();
    throw std::logic_error(msg.str());
  } else {
    values[&fluent()] = (*vi).second - expression().value(values);
  }
...
struct EmptyEffect : public Effect {
  ...
  virtual void listAtoms(AtomSet& atomsS, AtomList& atomsL, bool dupes)
  const {};
...
/* ... */
void AddEffect::listAtoms(AtomSet& atomsS, AtomList& atomsL, bool dupes)
  const {
  std::pair<AtomSet::iterator, bool> it = atomsS.insert(&atom());
  if (it.second || dupes)
    atomsL.push_back(&atom());
}
...
void DeleteEffect::listAtoms(AtomSet& atomsS, AtomList& atomsL, bool dupes)
  const {
  std::pair<AtomSet::iterator, bool> it = atomsS.insert(&atom());
  if (it.second || dupes)
    atomsL.push_back(&atom());
...
void ConjunctiveEffect::listAtoms(AtomSet& atomsS, AtomList& atomsL, bool dupes)
                                  const {
  for (EffectList::const_iterator ei = conjuncts_.begin();
       ei != conjuncts_.end(); ei++) {
    (*ei)->listAtoms(atomsS, atomsL, dupes);
  }  
}
...
void ConditionalEffect::listAtoms(AtomSet& atomsS, AtomList& atomsL, bool dupes)
  const {
  condition().listAtoms(atomsS, atomsL, dupes);
  effect().listAtoms(atomsS, atomsL, dupes);
}
...
/* Fills the provided lists with a sampled state change for this
   effect in the given state. */
void ProbabilisticEffect::listAtoms(AtomSet& atomsS, AtomList& atomsL,
                                    bool dupes) const {
  if (size() != 0) {
    size_t n = size();
    for (size_t i = 0; i < n; i++) {
      effect(i).listAtoms(atomsS, atomsL, dupes);
    }
  }
}
...
/* Fills the provided lists with a sampled state change for this
   effect in the given state. */
void QuantifiedEffect::state_change(AtomList&, AtomList&, UpdateList&, const
                                    TermTable&, const AtomSet&, const ValueMap&)
  const {
...
/* Fills the provided lists with a sampled state change for this
   effect in the given state. */
void QuantifiedEffect::listAtoms(AtomSet& atomsS, AtomList& atomsL,
                                 bool dupes) const {
  effect().listAtoms(atomsS, atomsL, dupes);
}
