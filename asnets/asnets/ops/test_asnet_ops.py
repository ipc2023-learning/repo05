...
def test_gather_op_auto(seed, batch_size, out_w, clist, in_w_list):
    ...
    with tf.compat.v1.Session(graph=tf.Graph()):
        ...
        input_placeholders = tuple(
            tf.compat.v1.placeholder(dtype=tf.float32, shape=(None, in_w, chan_count))
            for in_w, chan_count in zip(in_w_list, clist))
        index_placeholders = tuple(
            tf.compat.v1.placeholder(dtype=tf.int64, shape=(out_w, )) for _ in in_w_list)
...
def _do_ref_checks_auto(impl_node,
    ...
    if check_grads:
        ...
        impl_sqr_node = tf.reduce_sum(input_tensor=tf.square(impl_node))
        ref_sqr_node = tf.reduce_sum(input_tensor=tf.square(ref_node))
        impl_grad_node = tf.gradients(ys=impl_sqr_node, xs=input_placeholders)
        ref_grad_node = tf.gradients(ys=ref_sqr_node, xs=input_placeholders)
    ...
    with tf.compat.v1.Session(graph=tf.Graph()):
        ...
        for in_w, chan_count, pool_sizes in zip(in_w_list, c_list,
            ...
            input_placeholders.append(
                tf.compat.v1.placeholder(tf.float32, (None, in_w, chan_count)))
