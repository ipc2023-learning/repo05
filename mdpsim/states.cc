...
/* Constructs a state from members */
State::State(const Problem &problem, const AtomSet &atoms, const ValueMap &values)
  : problem_(&problem), atoms_(atoms), values_(values) {
  goal_ = problem.goal().holds(problem.terms(), atoms_, values_);
  // May as well do this just in case it's not set. Don't think *my* code relies
  // on "goal achieved fluent" being consistent though.
  if (goal()) {
    const Fluent& goal_achieved_fluent =
      Fluent::make(problem.domain().goal_achieved(), TermList());
    values_[&goal_achieved_fluent] = 1;
...
State& State::next(const Action& action) const {
  ...
  // TODO: figure out some way to get back the reward
  return *next_state;
}
