...
/* Finds original name associated with a simple type. Will break for union
   (non-simple) types. */
const std::string &TypeTable::find_name(const Type &type) const {
  if (type.index_ == 0) {
    throw std::range_error("this is untyped (index = 0)");
  }
  return names_.at(type.index_-1);
}
